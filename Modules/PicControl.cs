...
namespace HRQ.Modules
{
    public partial class PicControl : UserControl
    {
        ...
        {
            string dirPath = App.GetInstance().SystemFoldPath+"\\001\\1";
            try
            {
                if(flag == 0)
                {
                    dirs = new List<string>(Directory.GetFiles(dirPath + "\\k", "*.JPG", System.IO.SearchOption.TopDirectoryOnly));

                }
                else
                {
                    dirs = new List<string>(Directory.GetFiles(dirPath + "\\h", "*.JPG", System.IO.SearchOption.TopDirectoryOnly));
                }
            }
            catch (Exception e)
            {
